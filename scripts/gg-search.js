...
const GRAPHQL_QUERY = `query Search($filters: SearchFiltersInput!, $pagination: PaginationInput!, $currentUrl: String!) {
   ...
   async function hentOgVisSide(page) {
    ...
    async function showSellerInfo(listingId) {
        try {
            const listingBody = { operationName: "GetListing", variables: { id: listingId }, query: GET_LISTING_QUERY };
            ...
            const profileBody = {
                operationName: "Search",
                variables: {
                    filters: { weapons: true, categoryFields: [], userIds: [user.id] },
                    pagination: { perPage: 36, page: 1 },
                    currentUrl: ""
                },
