...
async function fetchPriceFileSize() {
  ...
  const res = await fetch(PRICE_FILE_HEAD, {
    method: 'HEAD'
  });
  ...
  toggle.addEventListener('change', async () => {
    ...
    if (toggle.checked) {
      ...
      const confirmed = confirm(
        `Prishistorik kræver download og behandling af ca. ${mb} MB data.`
      );
      if (!confirmed) {
...
function formatPrice(amount, currency, trade_type) {
  ...
  if (amount === 0) {
    if (trade_type === "Gives væk") {
      return '<span class="price-free">Gives væk</span>';
    } else {
      return "Til salg";
    }
  }
...
function makeCard(doc) {
  ...
  card.innerHTML = `
    ...
    <div class="time-since-badge"></div>
    ${retailerBadgeHtml}
...
`;
  ...
  if (timestamp > 1000000000000) {
    const badge = card.querySelector('.time-since-badge');
    if (badge) {
      badge.textContent = formatTimeSince(timestamp);
    }
  }
  return card;
}
...
window.hentOgVisDBA = async function(term, category, bgMode = false) {
  ...
  try {
    ...
    for (const pg of pagePromises) {
      const {
        docs
      } = pg === 1 ? first : await fetchDBAPage(pg, term, category);
